...
(deftest test-fnv ()
  (multiple-value-bind (buffer output input) (hash-io-context)
    ...
    (is (= (cl-hashing::mfix #x6a297d225dc8acc8) (fnv1a/fixnum buffer 10)))))
...
(deftest test-xxhash/64 ()
  (multiple-value-bind (buffer output input) (hash-io-context)
    ...
    (is (= (cl-hashing::mfix 8666379929374662555) (xxhash/fixnum buffer 1)))
    (reset-context output input)
    (hash-simple-string output "asdf")
    (is (= 4708639809588864798 (xxhash/64 buffer (fast-io::output-buffer-len output))))
    (is (= (cl-hashing::mfix 4708639809588864798) (xxhash/fixnum buffer (fast-io::output-buffer-len output))))
    (reset-context output input)
